...
for (const prod of Productos){
    if (prod.id == 10){ 
        ...
        <select id ="select" name="combo">
            <option value="10"selected>090</option>
            <option value="10.1" >100</option>
            <option value="10.2">120</option>
            <option value="10.3">140</option>
        </select>
        </form>`
        Prod.append(medida)
        ...
        medida.addEventListener("change",(element)=>{
            ...
            if(Medida_nueva.id == 10){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=10
            }else if (Medida_nueva.id == 10.1){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=10.1
            } else if (Medida_nueva.id == 10.2){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=10.2
            } else if (Medida_nueva.id == 10.3){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=10.3
            } else{
