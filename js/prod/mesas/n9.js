...
for (const prod of Productos){
    if (prod.id == 9){ 
        ...
        <select id ="select" name="combo">
            <option value="9"selected>090</option>
            <option value="9.1" >100</option>
            <option value="9.2">120</option>
            <option value="9.3">140</option>
        </select>
        </form>`
        Prod.append(medida)
        ...
        medida.addEventListener("change",(element)=>{
            ...
            if(Medida_nueva.id == 9){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=9
            }else if (Medida_nueva.id == 9.1){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=9.1
            } else if (Medida_nueva.id == 9.2){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=9.2
            } else if (Medida_nueva.id == 9.3){
                n_precio=Medida_nueva.precio
                precio.innerHTML=`<p>$${new Intl.NumberFormat(['ban', 'id']).format(n_precio)}</p>`
                id_nuevo=9.3
            } else{
